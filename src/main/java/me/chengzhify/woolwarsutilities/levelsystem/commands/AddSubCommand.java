...
public class AddSubCommand extends SubCommand {
    ...
    public boolean execute(CommandSender sender, String[] args) {
        if (args.length < 4) {
            sender.sendMessage(PREFIX + ChatColor.RED + "用法: /wwl add <玩家> <level|exp> <数量>");
            return true;
        }
        ...
        LevelManager.asyncLoadPlayer(targetUUID, realName, data -> {
            ...
            if (type.equalsIgnoreCase("level")) {
                ...
                sender.sendMessage(ChatColor.translateAlternateColorCodes('&', PREFIX + "玩家 &a" + realName + "&7 的等级现在为 " + LevelFormatter.getColoredLevel(data.getLevel(), data.getIcon())));
