...
public class GameStateListener implements Listener {
    ...
    public void onStart(GameStartEvent event) {
        if (VoicechatImpl.voiceServerApi == null) {
            if (voiceLog) instance.getLogger().warning("[WoolWars - VoiceGroup] Voicechat API 未初始化！");
            return;
        }

        instance.getLogger().info("[WoolWars - VoiceGroup] GameStartEvent 触发，正在创建语音组...");
        ...
        for (Team team : teams) {
            ...
            Bukkit.getScheduler().runTask(WoolWarsUtilities.getInstance(), () -> {
                ...
                for (User member : team.getMembers()) {
                    ...
                    if (connection == null) {
                        if (voiceLog) instance.getLogger().info("[WoolWars - VoiceGroup] 玩家 " + member.getName() + " 未连接到语音服务器！");
                        continue;
                    }
        ...
        if (voiceLog) instance.getLogger().info("[WoolWars - VoiceGroup] 竞技场 " + event.getArena().getWorld().getName() + " 的所有队伍语音组创建完毕。");
    }
    ...
    public void onEnd(GameEndEvent event) {
        ...
        if (voiceLog) instance.getLogger().info("[WoolWars - VoiceGroup] GameEndEvent 触发，正在移除语音组...");
        ...
        for (Team team : teams) {
            ...
            for (User member : team.getMembers()) {
                ...
                if (voiceLog) instance.getLogger().info("[WoolWars - VoiceGroup] 已将 " + member.getName() + " 移出队伍语音组");
                member.send("§c游戏结束，你已退出队伍语音组。");
            }
